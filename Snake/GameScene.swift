...
//  Snake
//
//  Created by Misha Behersky on 1/13/20.
...
class GameScene: SKScene {
    ...
    private func initializeGameView() {
        ...
        let boardHeight = CGFloat(numRows)*cellSize
        let boardWidth = CGFloat(numColumns)*cellSize
        let rect = CGRect(x: -boardWidth / 2, y: -boardHeight / 2, width: boardWidth, height: boardHeight)
        gameBG = SKShapeNode(rect: rect, cornerRadius: 0.02)
        gameBG.fillColor = SKColor.orange
        gameBG.zPosition = 2
        gameBG.isHidden = true
        self.addChild(gameBG)

        createGameBoard(width: boardWidth, height: boardHeight)
    }
